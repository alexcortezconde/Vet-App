...
import { Star, MapPin, CheckCircle, Search as SearchIcon, Filter, MessageSquare, Clock, X, Calendar, ChevronLeft, Send, Map as MapIcon, List, LogOut } from 'lucide-react';
...
export const VetSearch: React.FC = () => {
  ...
  const [showChat, setShowChat] = useState<Vet | null>(null);
  ...
  return (
    <div className="space-y-8 pb-10 animate-in fade-in duration-700">
      ...
      {viewMode === 'list' ? (
          ...
          {filteredVets.length > 0 ? filteredVets.map(vet => (
            <div key={vet.id} className="bg-white dark:bg-darkCard p-6 rounded-5xl border border-white dark:border-slate-800 shadow-sm hover:shadow-xl transition-all group overflow-hidden">
              ...
              <div className="mt-6 flex gap-3">
                ...
                <button onClick={() => setShowChat(vet)} className="px-5 bg-crema dark:bg-slate-800 text-secondary dark:text-slate-400 rounded-2xl hover:bg-primary hover:text-white transition-all shadow-sm">
        ...
        <div className="bg-white dark:bg-darkCard h-[500px] rounded-5xl border-4 border-white dark:border-slate-800 shadow-xl overflow-hidden relative map-dots">
           ...
           <div className="absolute bottom-6 left-6 right-6 bg-white/90 dark:bg-darkCard/90 backdrop-blur-xl p-5 rounded-4xl shadow-2xl border border-white dark:border-slate-800 flex items-center gap-4">
              ...
              <div>
                <h4 className="font-black text-secondary dark:text-slate-100 text-sm">Mostrando veterinarias en Nuñez</h4>
                <p className="text-[10px] text-slate-400 font-bold uppercase">3 locales disponibles</p>
              </div>
           </div>
        </div>
      )}

      {/* Modal Agendar con Calendario PopUp */}
      {showBooking && (
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {['09:00', '10:30', '12:00', '15:30', '17:00'].map(h => (
                    <button key={h} className="py-3 bg-white dark:bg-darkCard border border-slate-100 dark:border-slate-800 rounded-2xl text-[11px] font-black hover:bg-primary hover:text-white transition-all shadow-sm dark:text-slate-300">
                      {h}
                    </button>
                  ))}
                </div>
              </div>
              <button onClick={() => setShowBooking(null)} className="w-full py-5 bg-secondary dark:bg-slate-700 text-white rounded-3xl font-black uppercase tracking-widest shadow-xl hover:bg-primary transition-all">Confirmar Cita</button>
            </div>
          </div>
        </div>
      )}
      ...
      {/* Modal Chat */}
      {showChat && (
        <div className="fixed inset-0 z-[200] bg-secondary flex flex-col sm:max-w-md mx-auto sm:h-[80vh] sm:top-20 sm:rounded-5xl sm:shadow-2xl overflow-hidden">
          <div className="p-6 bg-secondary flex items-center justify-between">
            <button onClick={() => setShowChat(null)} className="text-white"><ChevronLeft className="w-6 h-6" /></button>
            <div className="flex flex-col items-center">
              <span className="text-white font-black">{showChat.name}</span>
              <span className="text-[10px] text-emerald-400 font-black uppercase">En línea</span>
            </div>
            <div className="flex items-center gap-3">
               <img src={showChat.imageUrl} className="w-10 h-10 rounded-xl object-cover border-2 border-white/20" />
               <button onClick={() => setShowChat(null)} className="text-white/40 hover:text-rose-400 transition-colors"><X className="w-6 h-6" /></button>
            </div>
          </div>
          <div className="flex-1 bg-crema dark:bg-darkBg rounded-t-5xl p-6 space-y-4 overflow-y-auto no-scrollbar">
             <div className="max-w-[80%] bg-white dark:bg-darkCard p-4 rounded-3xl rounded-tl-none shadow-sm text-sm font-medium dark:text-slate-200">¡Hola! ¿En qué puedo ayudarte hoy con Bruno?</div>
             <div className="max-w-[80%] bg-primary text-white p-4 rounded-3xl rounded-tr-none shadow-lg ml-auto text-sm font-medium">Hola Dr, quería consultarle por el nuevo pienso de Bruno.</div>
          </div>
          <div className="p-6 bg-crema dark:bg-darkBg border-t border-slate-100 dark:border-slate-800">
             <div className="flex gap-3">
               <input type="text" placeholder="Escribe un mensaje..." className="flex-1 p-4 bg-white dark:bg-darkCard rounded-3xl shadow-sm focus:outline-none font-medium text-sm dark:text-slate-200" />
               <button className="p-4 bg-primary text-white rounded-full shadow-lg"><Send className="w-5 h-5" /></button>
             </div>
             <button onClick={() => setShowChat(null)} className="w-full mt-4 py-2 text-[10px] font-black text-slate-300 dark:text-slate-600 uppercase tracking-widest hover:text-rose-500 transition-colors">Finalizar Chat</button>
          </div>
        </div>
      )}
    </div>
